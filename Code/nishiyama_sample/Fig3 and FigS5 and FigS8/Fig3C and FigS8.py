...
X = pd.read_csv(path+"/data/ChanpionData/miceData/240214_df_afterMice_1.csv")
RMSE_wider = pd.read_csv(path+"/data/ChanpionData/addinfo/RMSEchangge_wider.csv")
print(RMSE_wider)
dName = ["d0","d0m","d1","d1m","d2","d2m","d3","d3m","d7","d7m"]
RMSEdata = RMSE_wider[dName]
Labels=pd.read_csv(path+"/data/ChanpionData/DTWclustLabel.csv")
...
for i in i_list:
    ...
    if i == first_zero_index:
        axes.plot(range(len(dName)),rmse_values,alpha=0.7,color=cm.Pastel2(Life[i]),label="Restored")
    elif i == first_one_index:
        axes.plot(range(len(dName)),rmse_values,alpha=0.7,color=cm.Pastel2(Life[i]),label="Severe")
    else:
        axes.plot(range(len(dName)),rmse_values,alpha=0.7,color=cm.Pastel2(Life[i]))

rmse_mean = rmse_array.mean(0)
axes.plot(range(len(dName)),rmse_mean,alpha=1,lw=3,color="k",label="Mean")
axes.set_ylim(-5, 50)
axes.set_xticks(range(len(dName)))
axes.set_title("RMSE change")
axes.legend()
axes.set_xticklabels(["$D_{0}$","$DTI_{0}$","$D_{1}$","$DTI_{1}$","$D_{2}$","$DTI_{2}$","$D_{3}$","$DTI_{3}$","$D_{7}$","$DTI_{7}$"], rotation=0, ha='center')
fig.savefig(path+"/results/ChanpionData/RMSE_allPatient_Both.png",bbox_inches='tight')
plt.close()
...
# axes[1].set_title("Each RMSE change",fontsize=14)
# axes[1].legend(fontsize=13)

# fig.suptitle("Change in Predictive Performance",fontsize=15,y=0.92)
# fig.supxlabel("Type of data added",fontsize=14)
fig.supylabel("Rooted Mean Squired Error (RMSE)",fontsize=14,x=0.08)
fig.savefig(path+"/results/ChanpionData/RMSE_allPatient_parallel.pdf",bbox_inches='tight')
plt.close()